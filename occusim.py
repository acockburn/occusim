  ...
  def create_events(self, kwargs):
    ...
    for step in steps:
      ...
      if (step + "start") in self.args:
        ...
        if span > 0:
          event = datetime.datetime.combine(self.date(), start) + datetime.timedelta(seconds=random.randrange(span))
        elif span == 0:
    ...
    for step in randoms:
      ...
      dspan = int(datetime.datetime.combine(self.date(), maxd).timestamp() - datetime.datetime.combine(self.date(), mind).timestamp())
